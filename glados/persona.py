"""Характер помощника: обращения, приветствия, реплики."""
...
class Persona:
    ...
    def startup(self) -> str:
        """Фраза при запуске — звучит один раз, поэтому пусть будет разной."""
        name = self.assistant_name
        intro = f"{name} на связи. " if name else ""
        return random.choice([
            f"{intro}Все системы активны. Я слушаю вас, {self.address}.",
            f"{intro}Система запущена и готова к работе, {self.address}.",
            f"{intro}Я в вашем распоряжении, {self.address}.",
            f"Загрузка завершена. Слушаю вас, {self.address}.",
        ])
    ...
    def greeting(self) -> str:
        ...
        return random.choice([
            ...
            f"Приветствую вас, {self.address}! Все системы в норме. Чем могу помочь?",
