...
def parse_when(text: str, now: datetime | None = None) -> tuple[datetime | None, str]:
    ...
    # Дату назвали явно — уважаем её, даже если момент уже прошёл.
    # Если названо только время и оно уже прошло — значит, имелся в виду завтрашний день.
    if target <= now and not explicit_date:
