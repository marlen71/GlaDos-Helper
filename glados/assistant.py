...
class Assistant:
    def __init__(self, cfg: Config, voice: bool = True):
        ...
        self.fuzzy_wake = bool(cfg.get_path("wake.fuzzy", True))
    ...
    def process(self, phrase: str, require_wake: bool = True) -> bool:
        ...
        body, woken = strip_wake(phrase, self.wake_words, fuzzy=self.fuzzy_wake)
        ...
        try:
            while self.running:
                ...
                if self.process(text, require_wake=require):
                    if self.speech:
                        self.speech.wait()
                    mic.drain()          # не слушаем собственный голос
