...
#: Порядковые числительные среднего рода для дней месяца («двадцать третье»)
_DAY_ORD = {
...
_TENS_PREFIX = {20: "двадцать", 30: "тридцать"}


def day_to_words(day: int) -> str:
    """День месяца порядковым числительным: 23 -> «двадцать третье»."""
    if day in _DAY_ORD:
        return _DAY_ORD[day]
    tens, ones = divmod(day, 10)
    prefix = _TENS_PREFIX.get(tens * 10, "")
    return f"{prefix} {_DAY_ORD.get(ones, str(ones))}".strip()


def date_to_words(d) -> str:
    return (f"{day_to_words(d.day)} {MONTHS_GEN[d.month - 1]} "
            f"{year_to_words(d.year)} года")
